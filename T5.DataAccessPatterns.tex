...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Can be assisted by code generation libraries (popular in \textit{Ruby on Rails} and \textit{Laravel}).
  \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Lost updates
    \item Inconsistent reads
  \end{itemize}
\end{frame}

\begin{frame}
  \textbf{Lost updates}: Users \texttt{A} and \texttt{B} read the same data, make changes and write it back. The changes made by \texttt{A} are lost when \texttt{B} writes its changes.

  \begin{figure}
    \includegraphics[width=0.6\textwidth]{images/T5/lost-update.pdf}
    \caption{Two users increment a counter at the same time, but only one of the increments is saved.}
  \end{figure}
\end{frame}

\begin{frame}
  \textbf{Inconsistent reads}: User \texttt{A} performs 2 reads while \texttt{B} is modifying it. Each read is consistent, but the combination of the two reads is not.

  \begin{figure}
    \includegraphics[width=0.6\textwidth]{images/T5/inconsistent-read.pdf}
    \caption{A user reads a value twice in the same operation and gets two different results.}
  \end{figure}
\end{frame}
...
\begin{frame}
  ...
  \begin{figure}
    \includegraphics[width=0.7\textwidth]{images/T5/pessimistic-locking.pdf}
  \end{figure}
\end{frame}
...
\begin{frame}
  ...
  \only<1>{
    \begin{figure}
      \includegraphics[width=0.7\textwidth]{images/T5/optimistic-locking-1.pdf}
    \end{figure}
  }
  \only<2>{
    \begin{figure}
      \includegraphics[width=0.7\textwidth]{images/T5/optimistic-locking-2.pdf}
    \end{figure}
  }
...
\begin{frame}
  \frametitle{Offline concurrency | Example: Black Friday flash sale}

  \begin{figure}
    \includegraphics[width=0.8\textwidth]{images/T5/offline-transaction-reserved-stock.png}
    \caption{Reserved stock timer during a flash sale. Source: \href{https://creativethemes.com/blocksy/docs/woocommerce/cart-reserved-timer/}{Blocksy documentation}}
  \end{figure}
\end{frame}
