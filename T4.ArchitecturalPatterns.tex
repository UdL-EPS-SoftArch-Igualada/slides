...
\begin{frame}
  ...
  \textbf{The Solution:} For each API endpoint, write one procedure (function) that handles the entire transaction. 
  
  If procedures get too big, or the same code gets repeated (DRY), extract smaller helper functions.
  \newline
  ...
  \begin{itemize}
    ...
    \item It's the only option in \textbf{procedural languages} like C.
  \end{itemize}
\end{frame}
