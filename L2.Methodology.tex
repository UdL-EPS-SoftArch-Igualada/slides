...
\begin{frame}
  ...
  We will use a hybrid methodology called \textbf{Scrumban}\footnote{\url{https://www.atlassian.com/agile/project-management/scrumban}}, which combines elements of Scrum and Kanban:
...
\begin{frame}
  ...
  \begin{alertblock}{Warning}
    Nonsensical, AI-generated, or spammy tasks will incur a penalty of several points.
  \end{alertblock}
\end{frame}
...
\begin{frame}
  ...
  \begin{enumerate}
      ...
      \only<3>{
        \setcounter{enumi}{2}
      \item See which \textbf{HTTP methods} make sense for each URI.
      }
  \end{enumerate}
...
\begin{frame}
  ...
  \begin{itemize}
    \item The request body is a \textbf{JSON object}\footnote{The REST specification doesn't require the body to be JSON, but it's used almost universally in practice.}, and can only be used in \texttt{POST}, \texttt{PUT}, and \texttt{PATCH} (never in \texttt{GET} or \texttt{DELETE}).
    \item The response body is a \textbf{JSON object or array}.
...
\begin{frame}
  ...
  \begin{block}{Problem}
    What if I need to add functionality that doesn't map well to any resource URI? For example: "Resend the user's verification email"
  \end{block}
