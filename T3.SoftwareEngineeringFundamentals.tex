...
\begin{frame}
  ...
  \begin{exampleblock}{Corollary: \textbf{Don't Repeat Yourself (DRY) principle}}
    Every piece of knowledge must have a single, unambiguous, authoritative representation within a system.
  \end{exampleblock}

  In other words, \textbf{do not \textit{ever} copy and paste} a method, unless it is absolutely necessary and justified.
  \newline

  If you duplicate logic, whenever a change is made, you will need to:
  \begin{enumerate}
    \item Know/remember that someone duplicated that code (the compiler will not help you).
    \item Perform the same change a second time.
  \end{enumerate}
...
\begin{frame}
  ...
  Prepare a sheet of paper. You have \textbf{5 seconds} to memorize the following image.
  
  After that, you will have to reproduce it from memory.
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Hard limit: at most \textbf{2 levels of nesting} per method (for+for, for+if, while+try, etc.).
  \end{itemize}
...
\begin{frame}
  \frametitle{Liskov Substitution Principle: examples}
  \begin{columns}
    \column{.6\textwidth}
      \includegraphics[width=1\textwidth]{images/T3/liskov-2.pdf}
    \column{.5\textwidth}
  ...
  The possibility of \textit{throwing an exception} is a postcondition. If the users of the superclass are not expecting it, then the subclass is not substitutable.
\end{frame}
...
\begin{frame}
  ...
  In such a system, adding new functionality would never cause a regression on existing features.
  \newline
...
\begin{frame}
  ...
  Until now, we have seen \textbf{Design Principles}. They are \textit{general guidelines} that tell us how a well-designed system should look like.
  \newline
